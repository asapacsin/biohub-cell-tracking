#!/usr/bin/env bash
# GPU: edge_threshold=0.40 under promoted motion-relink-OFF recipe.
# Control scores (edge=0.45, motion OFF): fixed-8 0.915193 / holdout 0.962739
# from edge_thresh_0_45_motion_off_v1.
set -euo pipefail

SRC="${1:-/home/mc46451/biohub-cell-tracking}"
LOG_DIR="${SRC}/logs/slurm"
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/edge_thresh_0_40_motion_off.log"
...
echo "Starting edge_thresh 0.40 motion-off at $(date -Iseconds)" | tee "${LOG}"
...
| srun -p gpu_batch -N1 -n1 --gres=gpu:1 --cpus-per-task=8 --mem=64G -t 0-04:00:00 \
  ...
  bash -lc "
set -euo pipefail
DEST=/tmp/\${USER}/biohub-edge-thresh-040
OUT_NFS=\${HOME}/biohub-outputs/experiments/edge_thresh_0_40_motion_off_v1
rm -rf \"\$DEST\"
mkdir -p \"\$DEST\"
tar xf - -C \"\$DEST\"
cd \"\$DEST\"
...
CFG = Path('configs/experiments/edge_thresh_0_40_motion_off_det0_96875.yaml').resolve()
OUT = Path('outputs/experiments/edge_thresh_0_40_motion_off_v1').resolve()
OUT_NFS = Path(__import__('os').environ['OUT_NFS']).resolve()

CONTROL_FIXED = 0.9151933585182082
CONTROL_HOLD = 0.9627392206380307
HOLDOUT8 = [
    '44b6_0c582fdc', '44b6_0db75fae', '44b6_12dfb391', '44b6_144b256d',
    '6bba_062c8d37', '6bba_07477033', '6bba_07e24132', '6bba_085bf656',
]
...
def run_split(name, datasets, control_score):
    ...
    write_metric_outputs(
        ...
        {
            'schema_version': 1,
            'experiment': 'edge_thresh_0_40_motion_off_v1',
            'split': name,
            'config': str(CFG),
            'raw_geff': str(raw_dir),
            'note': 'GPU inference; motion_relink OFF; edge_threshold 0.40',
        },
    )
...
comparison = {
    'hypothesis': 'Lowering edge_threshold 0.45→0.40 under motion-relink OFF recovers remaining near-threshold true edges net-positive',
    'control_fixed8_motion_off_edge_0_45': CONTROL_FIXED,
    'control_holdout8_motion_off_edge_0_45': CONTROL_HOLD,
    'fixed8': {
...
comparison['decision'] = (
    'PROMOTE edge_threshold=0.40'
    if promote
    else 'KEEP edge_threshold=0.45; 0.40 not justified'
)
...
mkdir -p "${SRC}/outputs/experiments/edge_thresh_0_40_motion_off_v1"
srun -p gpu_batch -N1 -n1 --gres=gpu:1 -t 00:10:00 --mem=8G --chdir=/tmp bash -lc '
  cd ${HOME}/biohub-outputs/experiments/edge_thresh_0_40_motion_off_v1 && \
  tar cf - DONE comparison.json experiment_config.yaml \
    ...
    2>/dev/null
' | tar xf - -C "${SRC}/outputs/experiments/edge_thresh_0_40_motion_off_v1"
echo "LOGIN_MIRROR_OK $(date -Iseconds)" | tee -a "${LOG}"
cat "${SRC}/outputs/experiments/edge_thresh_0_40_motion_off_v1/comparison.json"
