#!/usr/bin/env bash
# Stage once, then run two-seed fixed-8 for each detection threshold (full re-inference).
# Does not modify outputs/fixed8/two_seed_alpha_0_5 (baseline preserved).
set -euo pipefail
...
LOG="${LOGIN_LOG_DIR}/two_seed_det_threshold_sweep.log"
...
[[ -f "${SUPPORT_DIR}/weights/unet_transformer/seed_314159/edge_predictor_best.pth" ]] \
  || { echo "Missing Seed 2 weights" >&2; exit 2; }
...
for t in "${THRESHOLDS[@]}"; do
  ...
  [[ -f "${SRC}/${cfg}" ]] || { echo "Missing config: ${cfg}" >&2; exit 2; }
done

echo "Starting two-seed det-threshold sweep at $(date -Iseconds)" | tee "${LOG}"
echo "Thresholds: ${THRESHOLDS[*]}" | tee -a "${LOG}"
...
h1 = hashlib.sha256(seed1.read_bytes()).hexdigest()
h2 = hashlib.sha256(seed2.read_bytes()).hexdigest()
assert h1 == "12f6881ee3620a831697ca098ff8f48e687a24225f4e048b538deec3562fe771", h1
assert h2 == "9bac2fa0dadc4a6fc1899e0caf187f4b553e0a7cd90ba1261a68b35ffe9e305f", h2
print("hashes_ok")
PY

      # Keep as strings so slugs stay 0_960 / 0_980 (not 0_96 / 0_98).
      THRESHOLDS=(0.955 0.960 0.965 0.96875 0.9725 0.975 0.980)
      FAILED=0
      for t in "${THRESHOLDS[@]}"; do
        slug="${t//./_}"
...
# String forms match directory slugs exactly.
thresholds = ["0.955", "0.960", "0.965", "0.96875", "0.9725", "0.975", "0.980"]
...
# Prefer consistency vs existing NFS baseline artifacts if present; else vs sweep 0.96875.
consistency = {}
...
if best:
    print(
        f"BEST threshold={best['detection_threshold']} "
        f"score={best['score']:.12f} "
        f"delta={best['delta_vs_two_seed_baseline']:+.12f}"
    )
else:
    print("BEST none")
PY
