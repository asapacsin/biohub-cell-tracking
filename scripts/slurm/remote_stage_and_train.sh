#!/usr/bin/env bash
# Remote half of stage_and_train.sh (runs on GPU node via srun bash -s).
set -euo pipefail
BASE="${1:?base dir name}"
DEST="/tmp/${USER}/${BASE}"
OUT_NFS="${HOME}/biohub-outputs"
mkdir -p "/tmp/${USER}" "${OUT_NFS}" logs 2>/dev/null || true

echo "host=$(hostname) gpu=$(nvidia-smi -L | head -1)"
echo "unpacking to ${DEST}"
rm -rf "${DEST}"
mkdir -p "${DEST}"
# Read tar stream from stdin into /tmp/$USER/
tar xf - -C "/tmp/${USER}"
cd "${DEST}"
date -Iseconds > .stage_complete
du -sh . data/competition 2>/dev/null || du -sh .
...
mkdir -p artifacts/detector outputs/architecture_learned logs/slurm
echo "=== train detector ==="
...
print("LEARNED_OK", "nodes", (df.row_type=="node").sum(), "edges", (df.row_type=="edge").sum())
PY

# Persist artifacts to NFS home (visible later from login via srun cat/cp).
mkdir -p "${OUT_NFS}/artifacts/detector" "${OUT_NFS}/submission" "${OUT_NFS}/logs"
...
cp -f artifacts/detector/seed_*.json "${OUT_NFS}/artifacts/detector/" 2>/dev/null || true
