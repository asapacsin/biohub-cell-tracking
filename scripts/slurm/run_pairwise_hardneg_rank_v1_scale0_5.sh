#!/usr/bin/env bash
# GPU eval: 0.5×-weight salvage of pairwise hard-neg ranking under frozen recipe C.
# Same dens/gap gates as v1; weights halved. Does not change motion/edge/det/short-track.
# Control: fixed-8 0.9181439782806684 / holdout-8 0.9646726188580379
# ETA: ~45 min wall (v1 finished in ~33 min)
set -euo pipefail

SRC="${1:-/home/mc46451/biohub-cell-tracking}"
LOG_DIR="${SRC}/logs/slurm"
mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/pairwise_hardneg_rank_v1_scale0_5.log"
...
echo "Starting pairwise_hardneg_rank_v1_scale0_5 at $(date -Iseconds)" | tee "${LOG}"
echo "ETA_WINDOW_OPEN=$(date -Iseconds) ETA_HOURS=0.75" | tee -a "${LOG}"
...
| srun -p gpu_batch -N1 -n1 --gres=gpu:1 --cpus-per-task=8 --mem=64G -t 0-06:00:00 \
  ...
  bash -lc "
set -euo pipefail
DEST=/tmp/\${USER}/biohub-pairwise-hardneg-v1-scale0-5
OUT_NFS=\${HOME}/biohub-outputs/experiments/pairwise_hardneg_rank_v1_scale0_5
rm -rf \"\$DEST\"
mkdir -p \"\$DEST\"
tar xf - -C \"\$DEST\"
cd \"\$DEST\"
...
CFG = Path('configs/experiments/recipe_c_edge_0_40_pairwise_hardneg_v1_scale0_5.yaml').resolve()
OUT = Path('outputs/experiments/pairwise_hardneg_rank_v1_scale0_5').resolve()
...
def run_split(name, datasets, control_score):
    ...
    summary = {
        **aggregate,
        'label': f'{name}_pairwise_hardneg_rank_v1_scale0_5',
        'edge_threshold': 0.40,
...
echo \"GPU pairwise_hardneg_rank_v1_scale0_5 finished at \$(date -Iseconds)\"
" 2>&1 | tee -a "${LOG}"

echo "Mirroring compact artifacts to login outputs/ ..." | tee -a "${LOG}"
mkdir -p "${SRC}/outputs/experiments/pairwise_hardneg_rank_v1_scale0_5"
srun -p gpu_batch -N1 -n1 --gres=gpu:0 --cpus-per-task=2 --mem=8G -t 0-00:20:00 --chdir=/tmp \
  bash -lc 'cd ${HOME}/biohub-outputs/experiments/pairwise_hardneg_rank_v1_scale0_5 && tar --exclude="*/predictions/*" --exclude="*/raw_geff/*" -czf - comparison.json DONE fixed8/summary.json fixed8/per_dataset.csv fixed8/command.txt fixed8/DONE holdout8/summary.json holdout8/per_dataset.csv holdout8/command.txt holdout8/DONE' \
  > /tmp/pairwise_hardneg_scale0_5_mirror.tar.gz 2>>"${LOG}" || true
if [[ -s /tmp/pairwise_hardneg_scale0_5_mirror.tar.gz ]]; then
  tar -xzf /tmp/pairwise_hardneg_scale0_5_mirror.tar.gz -C "${SRC}/outputs/experiments/pairwise_hardneg_rank_v1_scale0_5" || true
fi
echo "Finished pairwise_hardneg_rank_v1_scale0_5 at $(date -Iseconds)" | tee -a "${LOG}"
